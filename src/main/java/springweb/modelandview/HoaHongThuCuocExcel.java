...
public class HoaHongThuCuocExcel extends AbstractPOIExcelView {
	...
	private XSSFSheet templateSheet = null;
	...
	protected void buildExcelDocument(Map model, Workbook workbook, HttpServletRequest request,
		...
		File file = new File(System.getProperty("user.dir") + File.separator + "template" + File.separator
				+ "hoa_hong_thu_cuoc.xlsx");
		System.out.println("Final filepath : " + file.getAbsolutePath());
		month = (String) model.get("month");
		bill_cycle_id = (String) model.get("bill_cycle_id");
		template = new XSSFWorkbook(new BufferedInputStream(new FileInputStream(file)));
		templateSheet = template.getSheet("hoahong");
		XSSFSheet destSheet = (XSSFSheet) workbook.createSheet("hoahong");
	...
	private void buildHoaHongThuCuocSheet(Workbook workbook, HoaHongThuCuoc hhtc) {
		...
		XSSFSheet destSheet = (XSSFSheet) workbook.createSheet(hhtc.paramArray[HoaHongThuCuoc.COLLECTION_GROUP_ID]);
		Util.copySheets(destSheet, templateSheet, true);
		int row = -1, column = 1;
		XSSFRow srcRow;
		...
		for (HoaHongThuCuocTrongDs trongDs : hhtc.trongDs) {
			...
			if (k > 0) {
				...
				destRow = destSheet.getRow(row + 1);
				if (destRow == null)
					destRow = destSheet.createRow(row + 1);
